...
from keras.layers import AveragePooling2D, Conv2D, BatchNormalization, Activation, UpSampling2D, Input, Concatenate
...
class PSPNet(SegmentationModel):
    ...
    def build(self):
        ...
        for i in range(self.blocks):
            ...
            # Assign an aux output if there is room and it isn't the same at the main output
            # Up sample to match input shape
        ...
        x = pyramid_module(x)
        upsample_scalar = int(self.input_shape[-2] / x._keras_shape[-2])
        # Conv with 5 filters for 5 classes (in practice set) - change as required
...
def pyramid_branch(inputs, pooled_size, activation=None, batch_normalisation=False, n_branches=4):
    if inputs.shape[1] != inputs.shape[2]:
        raise ValueError(f"Inputs should be square but found to have shape {inputs.shape}")
    ...
    x = AveragePooling2D(pool_size=pool_size, padding='same', name='pool_' + name)(inputs)
    x = Conv2D(int(int(x.shape[-1]) / n_branches), (1, 1), padding='same', name='conv_' + name)(x)
    if batch_normalisation:
        x = BatchNormalization(name='bn_' + name)(x)
    if activation:
        x = Activation(activation, name=activation + '_' + name)(x)
    x = UpSampling2D((pool_dim, pool_dim), interpolation='bilinear', name='upsamp_' + name)(x)
    return x


def pyramid_module(inputs, activation=None, batch_normalisation=False, pool_sizes=(1, 2, 5, 10)):
    pyramid_outputs = []
